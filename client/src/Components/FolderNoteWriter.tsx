...
export default function FolderNoteWriter({note, handleNoteView, folderID} : {note :Note | undefined, handleNoteView : (value : boolean) => void, folderID : string | undefined}) {
    ...
    return <form className="flex flex-col w-full md:w-3/5 border-1 border-teal-200 h-full overflow-hidden" onSubmit={handleSubmit}>
        ...
        <Div cn=" h-6/7 border-1">
            <textarea className="text-sm resize-none scrollbar md:text-xl w-[95%] md:w-[97%] h-[96%] p-2 m-2 md:m-3 outline-none" ref={messageRef} placeholder="Start Writing...."></textarea>
        </Div>
    </form>
}